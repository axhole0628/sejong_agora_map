...
el('details').innerHTML=`<h2>${esc(b.name)}</h2><p>${esc(b.address)}</p><span class="metric">대장 표기 총 ${b.units.toLocaleString()}호</span><p>가구수 ${b.households} · 세대수 ${b.dwellings}</p>${b.appliedUnits!==null?`<p class="warning">분석 적용 호수: <b>${b.appliedUnits}호</b><br>${esc(b.correctionReason)}</p>`:''}<p class="source">${esc(b.unitSource)} · ${esc(b.document)}<br>대장 발급일: ${esc(b.ledgerDate)}</p><h3>${esc(meta.label)} 수록 업소 ${tenantShops.length}곳${peers.length>1?' (동일 주소 전체)':''}</h3>${peers.length>1?'<p class="warning">이 주소에 대장 '+peers.length+'건이 있습니다. 업소를 개별 동에 임의 배분하지 않았습니다.</p>':''}<div id="peer-buildings"></div><p class="muted">업소 수와 대장 호수의 차이는 실제 공실 수를 의미하지 않습니다.</p><div id="tenant-list" class="result-list"></div>`;
...
el('building-list').onclick=()=>{el('details').innerHTML='<h2>건물 38건 · 대장 표기 호수</h2>';buildings.forEach(b=>el('details').append(button(b.name+' · '+b.units+'호',()=>{map.setLevel(3);map.panTo(new kakao.maps.LatLng(b.lat,b.lng));showBuilding(b)})))};
render();
